...
import { login, getUsers, saveUsers, setCurrentUser } from '@/lib/storage';
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setToast({ show: true, message: (err.detail || 'Server login failed') as string, type: 'error' });
      ...
      const localLogin = login(email, password);
      if (!localLogin.success) {
        // Account exists in backend but not local storage (e.g. older data was cleared).
  ...
  const handleGoogleLogin = async () => {
    if (!isSupabaseConfigured() || !supabase) {
      setToast({ show: true, message: 'Google sign-in is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to your .env and enable Google in Supabase Dashboard → Authentication → Providers.', type: 'error' });
      return;
    }
